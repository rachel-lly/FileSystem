...
public class FSFile {
    ...
    private IndexFile parent;//父目录

    private String modifyTime;//最后修改时间

    private Integer status;//文件状态 0--关闭 1--打开 2--有用户正在写 -1--文目录

    private LinkedList<IndexFile> children;//子目录

    public FSFile(Integer index, String path, Boolean isCatalog, Boolean isPublic,
                  FatBlock firstBlock, IndexFile parent, String modifyTime,
                  Integer status, LinkedList<IndexFile> children) {
        this.index = index;
        this.path = path;
